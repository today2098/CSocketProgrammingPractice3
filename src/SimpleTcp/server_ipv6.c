...
int main() {
    ...
    int sock0 = socket(PF_INET6, SOCK_STREAM, 0);
    if(sock0 == -1) {
        perror("socket()");
        return 1;
    }
    ...
    server.sin6_port = htons(12345);  // ポート番号．
    server.sin6_addr = in6addr_any;   // IPアドレス（ワイルドカード）．
    ...
    inet_ntop(AF_INET6, &client.sin6_addr, buf0, sizeof(buf0));
    printf("accept from [%s]:%u\n", buf0, ntohs(client.sin6_port));
    fflush(stdout);
